...
처음엔 지금까지 해온 것 처럼 임의의 page객체를 만들어서 테스트 하면 될 것이라고 생각하고 임의의  
page객체를 만들기 위해 검색을 했다 PageImpl 클래스를 활용해서 사용자가 객체를 만들 수 있었다.  
제대로 해결되지 않은 부분이라고 생각하지만 일단 해결 한 벙법에 대해 적어본다.
...
> 그 외  
[Mocking page.class하는 방법](https://stackoverflow.com/questions/60306567/mocking-raw-string-in-unit-testing-for-spring-boot-service-layer)  
[pageImpl 임의로 페이지 나누기](https://stackoverflow.com/questions/26720768/spring-data-pageimpl-not-returning-page-with-the-correct-size)